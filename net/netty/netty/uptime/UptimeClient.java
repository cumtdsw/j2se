...
public class UptimeClient {
    ...
    public void run() {
        ...
        bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
        	...
        	/**
        	 * 这个timeoutHandler的实现原理：
        	 * 它在channelOpen事件发生时，
        	 * 使用ctx.getPipeline().execute(Runnable task)执行一个线程
        	 * 这个线程会检测是否超时，如果超时，就调用SimpleChannelUpstreamHandler的fireExceptionCaught方法抛出异常
        	 * 这个异常就可以在uptimeHandler中被捕获
        	 */
